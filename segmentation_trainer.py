...
class SegmentationTrainer:
    ...
    def _train_single_epoch(self, epoch):
        ...
        for index, data in enumerate(progress_bar):
            ...
            progress_bar.set_description("TRAINING - Epoch: {}, inference time: {:.2f} ms, iou: {:.2f}, "
                                         "loss: {:.2f}".format(epoch, (time() - start) * 1000, iou_value, loss.item()))
            self._write_scalar_to_tensorboard(dice_value, epoch, index, loss)
    ...
    def validate(self, epoch):
        ...
        for index, data in enumerate(progress_bar):
            ...
            loss = self._loss(model_output, masks)
            dice_value, iou_value = self._get_metrics(masks, model_output)
            progress_bar.set_description("VALIDATION - Epoch: {}, inference time: {:.2f} ms, iou: {:.2f}, "
                                         "loss: {:.2f}".format(epoch, (time() - start) * 1000, iou_value, loss.item()))
            self._write_scalar_to_tensorboard(dice_value, epoch, index, loss)
    ...
    def _write_scalar_to_tensorboard(self, dice_value, epoch, index, loss, mode=DataMode.train):
        ...
        self._writer.add_scalar(f"{mode}IOU", iou, epoch * len(self._data_loader[DataMode.train]) + index)
